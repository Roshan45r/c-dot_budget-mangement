<?php 
    ...
    if(isset($_POST['form_data'])) :
        ...
        if($data_id != "") :
            ...
            echo "helllo";
            $sql ="INSERT INTO `all_data` (`category_id`, `title`, `bps_date`, `invoice_date`, `invoice_amount`, `approval_amount`, `balance_amount`,`country`) VALUES (2, '$user_title', '$user_bps_date', '$user_invoice_date', '$user_invoice_amount', '$user_approval_amount', '$user_balance_amount','$user_country')";
        	$con = mysqli_connect('localhost', 'root', '');
            mysqli_select_db($con, 'budget_management');
            if ($con->query($sql) === TRUE)
            {
